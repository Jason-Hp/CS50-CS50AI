...
class MinesweeperAI():
 ...
 # 1. Mark the cell as a move that has been made
        ...
        while change:
            ...
            for sentence in self.knowledge:
                mined = sentence.known_mines().copy()
                for cell in mined:
                    self.mark_mine(cell)
                    change = True
                safed = sentence.known_safes().copy()
                for cell in safed:
                    self.mark_safe(cell)
                    change = True
                    ...
                    if sentence != sentence2:
                        if sentence.cells.issubset(sentence2.cells):
                            if sentence.cells != sentence2.cells:
                                gg= sentence2.cells - sentence.cells
                                ggc = sentence2.count - sentence.count
                                newsen = Sentence(gg,ggc)
                                if newsen not in self.knowledge and newsen not in hold:
                                    hold.append(newsen)
                                    change = True
            self.knowledge.extend(hold)
